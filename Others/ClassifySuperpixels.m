...
sigma = 0.8;
min_area = 25;
%name_of_query = testList{1};
tempstr = [segDir query '.ppm'];
imwrite(im, tempstr);
...
for kk = 1:length(kvalues),
    ...
    %segfile = ['segment/temp_k' num2str(k) '.ppm'];
    segfile = [segDir query '.ppm'];
    % segment image                
...
for f = 1 : length(retrList)
    ...
    mask = major./sum(labelHist) > 0.95; %Try making it > 0.9 or >0.99 and analyse the result
    gtDB{f} = y(mask);
    feaDB{f} = ft(:,mask);
end
...
[osim, orders] = sort(segSim, 2, 'descend'); %This step takes time
